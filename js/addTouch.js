// ============================================================================
// Anders Tornblad (https://github.com/atornblad/addTouch)
//
// Modifications for AaltoWindow
// v0.1 Jari Kleimola 2011:
// - hold ctrlKey to add/remove sticky touches
// - hold shiftKey to move all sticky touches
// - hold both ctrl and shift keys to bypass touch generation (generate standard mouse events)
// - default emulation is "mozilla"
// - mozilla emulation fires MozTouchRelease in addition to MozTouchUp
// ============================================================================
...
(function() {
    ...
    while (match) {
	 ...
	 // -- defaults to mozilla emulation
	 // -- replace with "webkit" to change the default mode
    if (!addTouchValue) addTouchValue = "mozilla"; //{ return };
...
//        a.createContextMenuItem(ul, "Pan", "pan");
    ...
    a.mousedown = function (event) {
    	...
    	// -- pass mouse messages if both control and shift are held down
            ...
            if (dx * dx + dy * dy < 100) {
                ...
                if (event.ctrlKey) {
                    ...
                    if (a.emulation == "webkit") {
                        ...
                        e2.initEvent("MozTouchRelease", true, true);
                        e2.altKey = event.altKey;
                        e2.streamId = t.streamId;
                        e2.mozInputSource = 5;
                        ...
                        a.raiseByMethod("onMozTouchRelease", target, e2);
                    }
	...
	a.mousemove = function (event)
	{
		if (a.currentTouch)
		{
            ...
            if (a.mode.zoom) {
            ...
            } else if (a.mode.wiggle || event.shiftKey) {
                var index = 0;
                ...
                for (var key in a.touches) {
                    ...
                    if (a.currentTouch != touch) {
                        ...
                        if (a.mode.wiggle)
                        {
	                        offsetX = speed * 0.5 * Math.sin(radians);
	                        offsetY = speed * 0.5 * Math.cos(radians);
								}
								else
								{
									offsetX = -dx;
    ...
    a.mouseup = function (event)
	 {
		if (a.currentTouch)
		{
			...
			{
				a.currentTouch._addTouchDiv.className = "_addTouchSpot";
				a.currentTouch.sticky = true;
			}
			else
			{
				if (!a.currentTouch.sticky)
				{
					var t = a.currentTouch;
                  ...
                  a.raiseByMethod("onMozTouchRelease", target, e2);
					...
					var key = "id_" + a.currentTouch.streamId;
					delete a.touches[key];
				}
				else a.currentTouch._addTouchDiv.className = "_addTouchSpot";
			}
			a.currentTouch = null;
		}
		return false;
